...
B. Thresholded pillar mask within the original ROI.
...
E. Simplified full-surface corridor-band detection with a separate coverage axis.
...
def create_exact_processing_schematic(
    ...
    ax_b.set_title("B  Thresholded ROI")
    ax_b.set_axis_off()
    ...
    # -------------------------------------------------------------------------
    # E. Simplified full-surface band detection
    # -------------------------------------------------------------------------
    ...
    ax_e_image.set_title("E  Detected horizontal corridor bands")
    ...
    ax_e_coverage.axvline(
        result["row_cov_thresh_rel_hull"],
        linestyle="--",
        linewidth=0.9,
        color="gray",
    )
    ...
    ax_e_coverage.set_xlim(
        0,
        max(
            result["row_cov_thresh_rel_hull"] * 2.2,
            float(np.nanmax(result["coverage"])) * 1.05,
        ),
    )

    ax_e_coverage.set_xlabel("Coverage", fontsize=8)
