...
Key display change
------------------
Panel C shows the horizontal corridor support represented by the measured
first-to-last accessible-pixel span at each x-position. All accessible pixels
outside these measured supports are drawn black inside the convex hull.
"""
...
def process_cellpose_corridors(
    ...
    #
    # For each detected horizontal corridor and each x-position, the measured
    # confinement is represented by the vertical span between the first and
    # last accessible pixel. Pixels not belonging to these measured horizontal
    # spans are black in the schematic. This does not alter the exported width
    # profiles or the analysis mask.
...
def create_exact_processing_schematic(
    ...
    axes[2].set_title(
        "C  Horizontal corridor support used for measurement\n"
        "All ignored accessible pixels are shown in black"
    )
