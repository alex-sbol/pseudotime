"""
Reproduce the corridor-mask workflow used in the pseudolocation project.

Important:
- The Cellpose flow image is the geometry source used to create the pillar,
  convex-hull, accessible-space, cleaned-corridor, and rotated-corridor masks.
- The FixedPattern image is optional here. In the original dataset workflow it
  was passed as `background`, rotated for the common coordinate frame, and used
  for reference dimensions/visualization. It did not generate the corridor mask.
"""
...
def black_mask_exact(image):
    """
    Match corridor_mask.py:

        if RGB: rgb2gray
        otherwise: float32 and divide by 255 when max > 1
        threshold_otsu
...
def process_cellpose_corridors(
    ...
    """
    Reproduce the mask and corridor-processing logic from the project code.

    The Cellpose image is used for:
    1. Otsu thresholding into black/non-black pixels
    2. convex hull of the black pixels
    3. accessible-space mask: (~black) & hull
    4. remove_small_objects(min_size=16)
    5. remove_small_holes(area_threshold=64)
    6. nearest-neighbour rotation
    7. row-wise coverage and horizontal-band detection
    8. confinement-width extraction

    The optional reference image is only rotated for comparison/registration.
    """
    ...
    # Exact skimage calls used in corridor_mask.py.
    ...
    for y0, y1 in true_runs(retained_after_smoothing):
        ...
        # Exact positional filter later used by collect_corridors().
    ...
    for row in corridors.itertuples(index=False):
        ...
        for x in range(row.x0, row.x1 + 1, scan_step_px):
            ...
            if ys.size == 0:
                ...
                # Exact width definition from collect_corridors().
...
def _display_mask(accessible, hull, outside_hull_value=0.70):
    """
    Build an unambiguous grayscale visualization of the processed mask.

    Display values
    --------------
    0.00 : non-accessible pixels inside the convex hull, including pillars
    1.00 : accessible corridor space inside the convex hull
    0.70 : pixels outside the convex hull, excluded from all measurements

    The outside-hull region is intentionally gray instead of white so that it
    cannot be confused with accessible corridor space.
    """
...
def create_exact_processing_schematic(
    ...
    # A. Cellpose input actually used.
    ...
    # C. Accessible space and cleanup.
    axes[2].imshow(
        _display_mask(result["cleaned"], result["hull"]),
        cmap="gray",
        vmin=0,
        vmax=1,
        interpolation="nearest",
    )
    removed_y, removed_x = np.where(result["removed_small_objects"])
    filled_y, filled_x = np.where(result["filled_small_holes"])
    ...
    if removed_x.size:
        axes[2].scatter(
            ...
            s=2,
            marker="x",
            label="Removed accessible components <16 px",
        )
    if filled_x.size:
        axes[2].scatter(
            filled_x,
            ...
            s=2,
            marker=".",
            label="Filled enclosed regions <64 px",
        )
    if removed_x.size or filled_x.size:
        axes[2].legend(
            loc="lower center",
    ...
    axes[2].set_title("C  Cleaned accessible space\nGray: outside hull and excluded")
    ...
    axes[3].set_title(
        f"D  Rotated corridor mask ({result['rotation_deg']:+.2f}°)\nGray: outside hull and excluded"
    )

    # E. Row coverage and detected bands.
    axes[4].imshow(
        rotated_display,
        cmap="gray",
        vmin=0,
    ...
    threshold_x = (
        result["row_cov_thresh_rel_hull"] * image_width * 0.30
    )
    ...
    axes[5].set_xlim(max(0, x0 - 3), min(rotated_display.shape[1] - 1, x1 + 3))
    axes[5].set_ylim(min(rotated_display.shape[0] - 1, y1 + 3), max(0, y0 - 3))
    _draw_box(
        axes[5],
    ...
    if not valid.empty:
        ...
        shown = valid.iloc[
            np.linspace(0, len(valid) - 1, n_lines, dtype=int)
        ]
