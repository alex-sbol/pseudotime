...
# add bbox for BOTH normal and fallback cases
for i, geom in enumerate(corridors):
    if i >= len(pseudotime["corridors"]):
        break

    line_id = pseudotime["corridors"][i].get("line_id", [])
    width = pseudotime["corridors"][i].get("width", [1] * len(line_id))

    pseudotime["corridors"][i]["bbox"] = {
        **geom,
        "line_id": line_id,
        "width": width,
    }
