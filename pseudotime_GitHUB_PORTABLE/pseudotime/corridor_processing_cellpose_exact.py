...
def _display_mask(accessible, hull):
    display = np.full(accessible.shape, np.nan, dtype=float)
    display[hull.astype(bool)] = 0.0
    display[accessible.astype(bool)] = 1.0
    return display
...
def create_exact_processing_schematic(
    ...
    axes[2].set_title("C  Accessible space inside hull after cleanup")
    ...
    axes[3].set_title(
        f"D  Corridor mask rotated by {result['rotation_deg']:+.2f}°"
    )
